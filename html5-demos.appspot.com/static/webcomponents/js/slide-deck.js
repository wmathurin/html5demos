...
SlideDeck.prototype.onDomLoaded_ = function(e) {
  ...
  this.slides = this.container.querySelectorAll('slide:not(.hiddendf13):not([hidden]):not(.backdrop)');
...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (settings.title) {
    document.title = settings.title.replace(/<br\/?>/, ' ').replace(/&lt;/g, '<');
...
SlideDeck.prototype.enableFrame_ = function(frame) {
  // TODO: upstream src doc addition.
  var srcdoc = frame.dataset.srcdoc;
  if (srcdoc && frame.src != srcdoc) {
    frame.srcdoc = srcdoc;
  }
  // end upstream
